...
test_that("something_but rule works", {
  ...
  expect_true(grepl(something_but(value = "a", mode = "lazy"), "b"))
  ...
  expect_true(
    compare(regmatches("abc", gregexpr(something_but(value = "a", mode = "lazy"), "abc"))[[1]], c("b", "c"))$equal
  )

  # expect an error if invalid 'mode' is given
  expect_error(something_but(mode = "whatever"))
})

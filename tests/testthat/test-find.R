...
test_that("find_rule works", {
  ...
  expect_equal(rx() %>% rx_find(value = ""), new_rx("(?:)"))

  # expect a match
  expect_true(grepl(rx() %>% rx_find(value = "ABC"), "ABC"))

  # dont expect a match, case sensitive
  expect_false(grepl(rx() %>% rx_find(value = "ABC"), "abc"))

  expect_false(grepl(rx() %>% rx_find(value = "abc"), "ac"))

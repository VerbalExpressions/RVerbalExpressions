...
2. Then, match a `range` of characters, a through z and 0 through 9
3. Then, match `one_or_more` characters in the range
4. Then, match `with_any_case` thereby enabling case insensitive matching
...
2. Then, match `anything_but` a period
3. Then, enable `end_of_line` behavior to ensure we grab the last period
4. Finally, use `gsub` to replace `anything` we found after the last period with an empty character
